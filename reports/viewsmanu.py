...
class PDFReport(FPDF):
    ...
    def header(self):
        ...
        self.set_xy(self.l_margin + logo_width + 5, self.get_y())
...
@login_required
def relatorio_frequencia_alunos(request):
    filters = _get_report_filters(request)
...
def export_frequencia_pdf(request):
    ...
    pdf_output_bytes = pdf.output(dest=\'S\')
    response = HttpResponse(pdf_output_bytes, content_type=\'application/pdf\')
    response[\'Content-Disposition\'] = f\'attachment; filename="frequencia_{selected_class.name.replace(\
